...
public class Utilidadesdeclase {
	...
	// Despues del Select, añade los atributos o columnas que queremos obtener separados por una coma.
    ...
    try{      
    	...
    	while(resultado.next()) {
           ...
           // for(Field field : entityFields) {
            	...
            	for (int i=1; i<= metadataResultSet.getColumnCount(); i++) {
            		
            	
                try {
                    map.put(metadataResultSet.getColumnName(i), resultado.getString(metadataResultSet.getColumnName(i)));
                    System.out.println("Columna  "+metadataResultSet.getColumnName(i)+"  Valor  "+resultado.getString(metadataResultSet.getColumnName(i)));
				} catch (Exception e) {
					 System.out.print(e.toString());    } 
